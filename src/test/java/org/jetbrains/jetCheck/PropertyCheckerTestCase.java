...
abstract class PropertyCheckerTestCase extends TestCase {
  ...
  <T> PropertyFalsified checkFails(PropertyChecker.Parameters parameters, Generator<T> checker, Predicate<T> predicate) {
    try {
  ...
  <T> T checkGeneratesExample(Generator<T> generator, Predicate<T> predicate, int minimizationSteps) {
    return checkFalsified(generator, predicate.negate(), minimizationSteps).getMinimalCounterexample().getExampleValue();
  }

  <T> PropertyFailure<T> checkFalsified(Generator<T> generator, Predicate<T> predicate, int minimizationSteps) {
    ...
    */
    assertEquals(minimizationSteps, failure.getTotalShrinkingExampleCount()); // to track that framework changes don't increase shrinking time significantly on average
